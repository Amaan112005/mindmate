...
from utils.mood_analysis import analyze_mood_from_text, detect_keywords
import logging
...
def save_journal_entry(entry_type: str, content: str, mood_score: int) -> bool:
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, ("default_user", timestamp, entry_type, content, mood_score, keywords))
            conn.commit()
...
def get_recent_entries(limit: int = 5) -> list:
...
def show_journal_form():
    ...
    with st.form("journal_form", clear_on_submit=True):
        ...
        if submitted:
            ...
            if save_journal_entry(entry_type, content, combined_mood):
...
def show_journal_history():
    ...
    for entry in entries:
        ...
        with st.expander(f"{entry_type} • {timestamp.strftime('%b %d, %Y %I:%M %p')} • {mood_emoji}"):
            st.markdown(f"""
